...
editor_tree = ET.parse(
    '/Users/stevenbailey/UDI Development PG3/Nodeservers/udi-poly-jaguar-landrover-v3/profile/editor/editors.xml')
    ...
    for ranges in item:
        ...
        if 'prec' in ranges.attrib:
            ...
            if 'min' not in ranges.attrib or 'max' not in ranges.attrib:
                print('ERROR in editor',
                      item.attrib['id'], 'prec requires min and max')
                issues += 1
        ...
        if 'step' in ranges.attrib:
            ...
            if 'min' not in ranges.attrib or 'max' not in ranges.attrib:
                print('ERROR in editor',
                      item.attrib['id'], 'step requires min and max')
                issues += 1
...
nls = {}
with open('/Users/stevenbailey/UDI Development PG3/Nodeservers/udi-poly-jaguar-landrover-v3/profile/nls/en_us.txt') as fp:
    for line in fp:
        line = line.rstrip()
        if line != "" and line[0] != '#':
            pair = line.split(" = ")
...
node_tree = ET.parse(
    '/Users/stevenbailey/UDI Development PG3/Nodeservers/udi-poly-jaguar-landrover-v3/profile/nodedef/nodedefs.xml')
    ...
    for node in item:
        if node.tag == 'sts':
            for status in node:
                ...
                if name in nls:
                    print("status %s has name = %s" %
                          (status.attrib['id'], nls[name]))
                else:
                    print("ERROR: status %s name is missing or incorrect" %
                          status.attrib['id'])
